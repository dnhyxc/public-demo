...
const Guide:FC<PropTypes> = (props) => {
    ...
    const stepOneNormalContent =  [ // 普通员工
    ...
    const renderSubTitle = useCallback((content: string) => {
        return content.includes('<br/>') ? content.split('<br/>').map(item => <div>{item}</div>) : content
    }, [])
    ...
    useLayoutEffect(() => {
        const handleScroll = () => {
            ...
            if(run) {
                ...
                const box = document.querySelector('.react-joyride__spotlight')?.getBoundingClientRect()
                if(card && box) {
    ...
    const tooltipComponent = ({
        ...
        boxRef.current = document.querySelector('.react-joyride__spotlight')?.getBoundingClientRect()
        let image = GuideStep
